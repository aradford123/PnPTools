...
def get_status(apic, serial):

    response = apic.pnpsearch.getPnpAllSiteDevicesByRange(serialNumber=serial,deviceMatchesARule=True)
    return apic.serialize(response.response[0].projectDevice[0])
'''
{
...
'''
def watch_status(apic, serial, last_status):
    status = get_status(apic, serial)
    #print json.dumps(status, indent=2)
    if status is None:
        print("No site status for device")
        sys.exit(1)
    this_status_detail = status['stateDisplay']
    this_status = status['state']

    return this_status, this_status_detail
